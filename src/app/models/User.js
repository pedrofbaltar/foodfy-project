...
module.exports = {
  ...
  async delete(id) {
    try {
      let results = await await db.query(
        `SELECT recipes.* FROM recipes WHERE recipes.user_id=$1`,
        [id]
      );
      ...
      for (let index = 0; index < recipes.length; index++) {
        await db.query(
          `DELETE FROM recipes_files WHERE recipes_files.recipe_id = $1`,
          [recipes[index].id]
        );
      }
  ...
  async update(data) {
    try {
      ...
      let values = [data.name, data.email, passwordHash, isAdmin, data.id];
